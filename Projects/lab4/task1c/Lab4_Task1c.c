...
// Two virtual buttons are also generated on the LCD display which, when pressed,
// updates the clock frequency that the microcontroller is running on.
...
int main(void) {
  ...
  while (1) {
    ...
    updateTemp = 0;
    // create buttons
    LCD_DrawFilledRect(32, 120, 100, 60, Color4[14]); // yellow button on the left
    LCD_DrawFilledRect(192, 120, 100, 60, Color4[13]); // purple button on the right
    //touch screen
